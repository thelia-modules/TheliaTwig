...
use TheliaTwig\Template\TwigParser;
...
class TwigAssetsResolver implements AssetResolverInterface
{
    ...
    public function resolveAssetURL(
        ...
        $fileRoot = $this->resolveSourcePath($source, $sourceTemplateName, $file, $parserInterface);
    ...
    protected function resolveSourcePath(
        $source,
        $templateName,
        $fileName,
        ParserInterface $parserInterface,
        ...
        if ($isForRender && substr($fileName, strlen($fileName)-4) != "twig") {
            $fileName .= ".twig";
        }
    ...
    protected function getPossibleAssetSources($directories, $template, $source)
    {
        ...
        if (ParserInterface::TEMPLATE_ASSETS_KEY !== $source) {
            // We're in a module.
            ...
            if (isset($directories[$template][ParserInterface::TEMPLATE_ASSETS_KEY])) {
                $paths[] =
                    $directories[$template][ParserInterface::TEMPLATE_ASSETS_KEY]
                    . DS
            ...
            $paths[] = $directories[$template][ParserInterface::TEMPLATE_ASSETS_KEY];
        }
